...
def setup_training(self):
    ...
    self.EPSILON_PHASE_I  = False
    self.EPSILON_MAX_PHASE_I  = 0.1
    self.EPSILON_MIN_PHASE_I  = 0.05
    self.EPSILON_DECAY_I = 0.99999
    self.EPSILON_PHASE_II = True
    self.EPSILON_MAX_PHASE_II = 0.1
    self.EPSILON_MIN_PHASE_II = 0.05
    self.EPSILON_DECAY_II = 0.99999
    self.epsilon_I       = self.EPSILON_MAX_PHASE_I
    self.epsilon_II      = self.EPSILON_MAX_PHASE_II
    ...
    self.expert_history     = deque(maxlen=self.HISTORY_SIZE)

    # Performance parameters and analysis:
    file_name = "performance_" + datetime.now().strftime("%d_%m_%Y_%H_%M_%S") + ".txt"
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: list):
    ...
    # Epsilon decay:
    if self.PHASE_I:
        self.epsilon_I *= self.EPSILON_DECAY_I
    else:
        self.epsilon_II *= self.EPSILON_DECAY_II
    ...
    self.done_history.append(0) # 0 = False ; 1 = True
    if self.PHASE_I:
        self.expert_history.append(1) # 0 = False ; 1 = True
    else:
        self.expert_history.append(0) # 0 = False ; 1 = True
    ...
    if (self.step_count % self.STEPS_UPADTE_MODEL == 0) and len(self.state_history) >= self.BATCH_SIZE:
        ...
        expert_batch     = np.array([self.expert_history[i] for i in indices])
        ...
        l = 0.8
        L = np.full((self.BATCH_SIZE,len(self.ACTIONS)), l)
        L[expert_batch==0] = 0
        L[range(self.BATCH_SIZE),action_batch] = 0
        Q = np.where(Q >= Q[range(self.BATCH_SIZE),action_batch][:,np.newaxis]-l, Q-L, Q)
...
def end_of_round(self, last_game_state: dict, last_action: str, events: list):
    ...
    # Update episode_count and step_count:
    self.step_count += 1
    self.episode_steps += 1
    ...
    # Save Performance from Episode (Steps, Reward, Score, Phase):
    self.file.write("{},{},{},{}\n".format(self.episode_steps, self.episode_reward, last_game_state["self"][1], int(self.PHASE_I)))
    self.file.flush()
    os.fsync(self.file.fileno())

    # Reset episode_steps and episode_reward:
    self.episode_steps = 0
    self.episode_reward = 0
    ...
    self.reward_history.append(reward)
    self.done_history.append(1) # 0 = False ; 1 = True
    if self.PHASE_I:
        self.expert_history.append(1) # 0 = False ; 1 = True
    else:
        self.expert_history.append(0) # 0 = False ; 1 = True
    ...
    if (self.step_count % self.STEPS_UPADTE_MODEL == 0) and len(self.state_history) >= self.BATCH_SIZE:
        ...
        reward_batch     = np.array([self.reward_history[i] for i in indices])
        done_batch       = np.array([self.done_history[i] for i in indices])
        expert_batch     = np.array([self.done_history[i] for i in indices])
...
def determin_custom_events(old_game_state: dict, new_game_state: dict) -> list:
    ...
    if len(positions_crates_old) != 0:
        ...
        index = np.argmin(distances_crates_old)
        if distances_crates_old[index] > distances_crates_new[index]:
            custom_events.append(CLOSER_TO_CRATE)
        elif distances_crates_old[index] < distances_crates_new[index]:
    ...
    if len(positions_coins_old) != 0:
        ...
        index = np.argmin(distances_coins_old)
        if distances_coins_old[index] > distances_coins_new[index]:
            custom_events.append(CLOSER_TO_COIN)
        elif distances_coins_old[index] < distances_coins_new[index]:
    ...
    if len(positions_opponents_old) != 0:
        ...
        index = np.argmin(distances_opponents_old)
        if distances_opponents_old[index] > distances_opponents_new[index]:
            custom_events.append(CLOSER_TO_OPPONENT)
        elif distances_opponents_old[index] > distances_opponents_new[index]:
            custom_events.append(FARTHER_TO_OPPONENT)

    if 0 < state_to_feature_tensor(new_game_state)[63] <= 2:
        custom_events.append(DANGER_PENALTY)

    if np.array_equal(state_to_feature_tensor(old_game_state),state_to_feature_tensor(new_game_state)):
...
def reward_from_events(events: list) -> int:
    ...
    game_rewards = {
        ...
        e.SURVIVED_ROUND        :  0/N,

        CLOSER_TO_CRATE         :  +2/N,
        FARTHER_TO_CRATE        :  -1/N,
        CLOSER_TO_COIN          :  +5/N,
        FARTHER_TO_COIN         :  -1/N,
        CLOSER_TO_OPPONENT      :  +5/N,
...
# Code for expert (=rule_based_agent):
