...
def act(self, game_state: dict) -> str:
    ...
    if self.train:
        ...
        if self.PHASE_I:        
            ...
            if self.EPSILON_PHASE_I and np.random.uniform() < max(self.epsilon, self.EPSILON_MIN_PHASE_I):
        ...
        if self.PHASE_II:
            # Exploration with probability epsilon:
            if self.EPSILON_PHASE_II and np.random.uniform() < max(self.epsilon, self.EPSILON_MIN_PHASE_II):
